...
to setup-patches
  ...
  let N2 (%PatchA2 / 100 * (size-city * size-city))
  ask patches [set act1 1 set act2 0 set switchingParisians 0]
...
to GetActive
  doA2
   ask parisians [set JustSwitched? 0]
  comeBack
  ask parisians [set JustSwitched? 0]
...
to Update-Color
    ...
    ask patches with [propLocalVeggiePatch >= currentMeanVeggie] [set pcolor green]
end
